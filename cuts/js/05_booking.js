...
function selectOperator(opId) {
  ...
  document.getElementById('opchip-' + opId).classList.add('selected');
...
function updateBookSummary() {
  ...
  if (!date || !svcId || !opId) { summary.classList.add('hidden'); return; }
...
async function doBooking() {
  ...
  try {
    const result = await api('booking.create', {
      ...
      date, operatorId: STATE.selectedOp,
      serviceId   : svcId, notes,
    });
    openModal(`
      <div style="text-align:center;padding:20px 0">
        ...
        <div class="text-muted text-small" style="margin-top:8px">Operator: ${result.operatorName}</div>
